...
class th_getattach(threading.Thread):
	...
	dstfile=dstdir+"/"+self.filename
		...
		syn=self.xattr.getxattr(dstfile,'user.sync')
		if not syn:
			[msgid for msgid in msgintf if msgintf[msgid]((self,'touch',dstfile))]
			...
			with open(dstfile,"wb") as fhnd:
				...
				[msgid for msgid in msgintf if msgintf[msgid]((self,'write',dstfile))]
